...
for line in sys.stdin:
    ...
    if name == 'LDBG':
        ...
        print(f"  st_rd1=0x{bits(v,63,48):04X} (want 5A3C)  st_rd0=0x{bits(v,47,32):04X} (want A5C3)")
        print(f"  st_loops={bits(v,31,28)} st_state={bits(v,27,24)} wr_cnt=0x{bits(v,23,12):03X} rd_cnt=0x{bits(v,11,0):03X}")
    elif name == 'LCPU':
        pc = bits(v,55,33) << 1
